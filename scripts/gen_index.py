...
Contrairement à supports-informatique (arborescence SeqNN_.../TP_NN/), ce
dépôt organise ses documents par thème (dossiers de premier niveau type
01_conception, 02_TP/, [01]cours/...). Le regroupement se fait donc sur ce
dossier de premier niveau plutôt que sur une variable \\sequence numérique,
et la catégorie (Cours/TD/TP) est déduite de l'option de \\documentclass.
"""
import html
import re
import subprocess
...
def module(doc_path: str) -> str:
    """Dossier de premier niveau du document (regroupement thématique)."""
...
rows = defaultdict(lambda: defaultdict(list))  # rows[module][catégorie] = [(label, rel_pdf, date), ...]
...
for doc, rel in built:
    ...
    if cat == "Autre":
        autres[mod].append((label, rel, date))
    else:
        rows[mod][cat].append((label, rel, date))

    if doc in corriges:
        rows[mod]["Correction"].append((f"{label} (corrigé)", corriges[doc], date))
...
for mod in sorted(rows):
    cours = cell_html(rows[mod].get("Cours", []))
    td = cell_html(rows[mod].get("TD", []))
    tp = cell_html(rows[mod].get("TP", []))
    correction = cell_html(rows[mod].get("Correction", []))
    table_rows.append(f"""
      <tr>
        <th scope="row">{html.escape(mod)}</th>
        <td>{cours}</td>
        <td>{td}</td>
        <td>{tp}</td>
        <td>{correction}</td>
...
if autres:
    items = "\n".join(
        f'      <li>{cell_html([e])} <span class="mod-tag">({html.escape(mod)})</span></li>'
        for mod in sorted(autres) for e in autres[mod]
    )
    autres_html = f"""
    <section>
...
<head>
<meta charset="utf-8">
<title>Supports Automatisme &amp; Robotique - BUT GEII</title>
...
<style>
  ...
  .mod-tag {{ color: var(--muted); font-size: 0.85em; }}
...
<body>
  <h1>Supports Automatisme &amp; Robotique - BUT GEII</h1>
  <p class="subtitle">Tous les supports de cours, td et tp du module Automatisme &amp; Robotique - GEII IUT Annecy</p>
  <div class="table-wrap">
    <table>
      <caption>Tableau des supports</caption>
      <thead>
        <tr><th scope="col">Module</th><th scope="col">Cours</th><th scope="col">TD</th><th scope="col">TP</th><th scope="col">Correction</th></tr>
      </thead>
